...
try:
    ...
    logger.error(f"Missing dependencies: {e}. Please install langchain-community, chromadb, langchain-huggingface, sentence-transformers")
    sys.exit(1)
...
# Global reference to the vector store
vectorstore = None
...
def handle_request(request):
    ...
    # Call a Tool
        ...
        if name == "search_knowledge_base":
            ...
            # Retrieve metadata from Chroma to list unique sources
            data = vectorstore.get()
            unique_sources = {m.get("source") for m in data["metadatas"] if m.get("source")}
            ...
            if not data["documents"]:
                ...
                unique_sources = {m.get("source") for m in all_data["metadatas"] if m.get("source")}
